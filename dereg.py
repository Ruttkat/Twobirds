...
WORTH = [1,4,6,2,1,4,3,2,1,10,5,2,3,2,1,4,8,1,1,1,2,4,2,10,4,10]
WORTH_UNICODE = {
  u'Ä':7,
  u'Ö':7,
  u'Ü':7
...
def letterscore(l):
  if l in WORTH_UNICODE:
    return WORTH_UNICODE[l]
  else:
    return WORTH[ord(l)-ord('A')]


def wordscore(word):
  bonus = BONI[len(word[0])] 
  score = sum([letterscore(l) for l in word[0]])
